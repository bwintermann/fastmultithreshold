...
/** @file
 * @brief Joins a STL-like iterable T into a string with the provided delimiter.
 *
 * No general/easy way of printing a iterable with a delimiter exists in
 * standard C++. A costum infix output stream iterator takes care of the
 * formatting.
 */
 ...
 */
template <typename T>
auto join(const T& p, const char* delimiter) ->
typename std::enable_if<is_iterable<T>::value, std::string>::type {
    std::ostringstream s;
    std::copy(p.begin(), p.end(),
        infix_ostream_iterator<typename T::value_type>(s, delimiter));
    return s.str();
}
